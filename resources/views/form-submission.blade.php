    ...
    <div class="col-xl-10">
        <div class="card card-soft">
            <div class="card-body p-4">
                ...
                <form id="multiStepForm" method="POST" action="{{ route('form.store') }}">
                    @csrf
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    nextBtn.addEventListener('click', (e) => {
        ...
        // Immediately disable all date inputs to prevent picker
        const dateInputs = document.querySelectorAll('input[type="date"]');
        dateInputs.forEach(input => {
            input.disabled = true;
            input.classList.add('no-picker');
        ...
        // Set flag to prevent date picker from opening
        document.body.dataset.navigating = 'true';
        
        // Force close ALL date pickers
        closeAllDatePickers();
        
        // Small delay to ensure blur completes before step change
        setTimeout(() => {
            if (current < steps.length - 1) {
                current += 1;
                setStep(current);
            }
            // Re-enable date inputs after navigation
            setTimeout(() => {
                dateInputs.forEach(input => {
                    input.disabled = false;
                    input.classList.remove('no-picker');
    ...
    nextBtn.addEventListener('mousedown', (e) => {
        // Immediately disable all date inputs
        const dateInputs = document.querySelectorAll('input[type="date"]');
        dateInputs.forEach(input => {
            input.disabled = true;
            input.classList.add('no-picker');
    ...
    addFamilyMemberRow(); // start with one row

    // Prevent date inputs from opening when clicking Next/Previous buttons
    document.addEventListener('click', (e) => {
        // If clicking on Next/Previous buttons, close any open date pickers
    ...
    // Prevent date inputs from auto-opening during navigation or step change
    document.querySelectorAll('input[type="date"]').forEach(input => {
        // Prevent focus/click from opening date picker when navigating
        ...
        // Also prevent click from opening date picker when navigating
        ...
        // Prevent mousedown from opening picker
